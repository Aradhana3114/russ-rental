<header x-data="{ open: false, scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 20 })" class="sticky top-0 z-40 transition-all duration-300" :class="scrolled ? 'bg-white/95 backdrop-blur-3xl shadow-lg shadow-black/5 border-b border-brand/10' : '{{ request()->routeIs('home') ? 'bg-black/30 backdrop-blur-3xl border-b border-white/10' : 'bg-white/95 backdrop-blur-3xl shadow-md border-b border-brand/10' }}'">
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 shrink-0">
                ...
                <div class="leading-tight">
                    <span class="block font-serif text-xl font-semibold tracking-wide" :class="scrolled ? 'text-brand-dark' : '{{ request()->routeIs('home') ? 'text-white' : 'text-brand-dark' }}'">RUSS</span>
                    <span class="block text-[10px] tracking-[0.2em] uppercase" :class="scrolled ? 'text-brand/70' : '{{ request()->routeIs('home') ? 'text-white/70' : 'text-brand/70' }}'">Rental Mobil</span>
                </div>
            </a>

            <!-- Desktop Nav -->
            <nav class="hidden lg:flex items-center gap-8 text-sm font-medium" :class="scrolled ? 'text-brand-dark' : '{{ request()->routeIs('home') ? 'text-white' : 'text-brand-dark' }}'">
                @php
                    $navLinks = [
                ...
                @foreach ($navLinks as $key => $link)
                    <a href="{{ route($link['route']) }}"
                       ...
                       :class="scrolled ? '{{ request()->routeIs($link['route']) ? 'text-brand font-semibold' : 'hover:text-brand' }}' : '{{ $isHome ? (request()->routeIs($link['route']) ? 'text-brand-light font-semibold' : 'hover:text-white') : (request()->routeIs($link['route']) ? 'text-brand font-semibold' : 'hover:text-brand') }}'">
                        {{ $link['label'] }}
                    </a>
                @endforeach
            </nav>
            ...
            <!-- Mobile toggle -->
            <button @click="open = !open" class="lg:hidden p-2" :class="scrolled ? 'text-brand-dark' : '{{ request()->routeIs('home') ? 'text-white' : 'text-brand-dark' }}'">
    ...
    <!-- Mobile Nav -->
    <div x-show="open" x-cloak x-transition class="lg:hidden border-t border-brand/10 bg-white/95 backdrop-blur-3xl">
        <nav class="flex flex-col px-4 py-4 gap-1 text-sm font-medium text-brand-dark">
